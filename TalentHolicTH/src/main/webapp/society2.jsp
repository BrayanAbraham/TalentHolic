...
<%String email=request.getParameter("societyemail");
String pass="xyz";
SocietyDetailsService sds=new SocietyDetailsService();
SocietyDetailsEntity sde=sds.getUser(email, pass);
						...
						<div id='posts' >
							...
							<div class="entry clearfix`center" style="padding: 10px;border: 1px solid #dcd8d8; box-sizing: border-box;">
								...
								<div class="entry-title center"><% if((user!=null)||soc!=null){%>
									<a href="like?id=<%=pe.id%>&url=society2.jsp"><i class="icon-arrow-up icons"></i><%=le.like %></a>&nbsp;/&nbsp;<a href="dislike?id=<%=pe.id%>&url=society2.jsp"><i class="icon-arrow-down icons"></i><%=le.dislike %></a>&nbsp;/&nbsp;<%} %><a href="#"><i class="icon-folder-open"></i><%=pe.category %></a>&nbsp;/&nbsp;<a href="blog-single-full.html#comments"><i class="icon-comments"></i>Comment</a>&nbsp;/&nbsp;<a href="#"><i class="icon-share icons"></i>Share</a>
								</div>
								<%} }%>
								...
								<div class="comments" align="left">
								 ...
								 <%}}%>
									...
									<div>
										<form id="comment" action="comment?id=<%=pe.id %>&name=<%if(user!=null){ %><%=user.name %><%}else if(soc!=null){ %><%=soc.name %><%}else{ %><%="Anonymous" %><%} %>&url=society2.jsp" role="form" method="post" class="nobottommargin">
											<div class="col_full">
