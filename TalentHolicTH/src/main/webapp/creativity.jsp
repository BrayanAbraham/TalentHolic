								...
								<div id="post-list-footer">
									...
									if(lk!=null){
										...
										while(itd.hasNext())
										{
											...
											if(sde.category.equals("creativity")){%>
						...
						<div id="posts" >
							...
							<%
									...
									while(itr.hasNext())
									{
										...
										if(pe.category.equals("creativity")) {
										%>
							...
							<div class="entry clearfix`center" style="padding: 10px;border: 1px solid #dcd8d8; box-sizing: border-box;">
								...
								<div class="entry-title center"><% if((user!=null)||soc!=null){%>
									<a href="like?id=<%=pe.id%>&url=creativity.jsp"><i class="icon-arrow-up icons"></i><%=le.like %></a>&nbsp;/&nbsp;<a href="dislike?id=<%=pe.id%>&url=creativity.jsp"><i class="icon-arrow-down icons"></i><%=le.dislike %></a>&nbsp;/&nbsp;<%} %><a href="#"><i class="icon-folder-open"></i><%=pe.category %></a>&nbsp;/&nbsp;<a href="#"><i class="icon-comments"></i>Comment</a><%if((user!=null)||(soc!=null)){if((user.email.equals(pe.email))||(soc.email.equals(pe.email))||(user.auth==3)){ %>&nbsp;/&nbsp;<a href="delete?id=<%=pe.id%>&url=/profile.jsp"><i class="icon-close"></i>Delete</a><%} }%>
								</div>
								<%} }%>
								...
								<div class="comments " align="left">
								 ...
								 <%}}%>
									...
									<div>
										<form id="comment" action="comment?id=<%=pe.id %>&name=<%if(user!=null){ %><%=user.name %><%}else if(soc!=null){ %><%=soc.name %><%}else{ %><%="Anonymous" %><%} %>&url=creativity.jsp" role="form" method="post" class="nobottommargin">
											<div class="col_full">
